...
RUN cargo build --release --features ner || true
RUN rm -rf src

# Copy real source and compile release binary
COPY src/ ./src/
RUN cargo build --release --bin blindpipe --features ner
...
COPY --from=builder /usr/src/app/target/release/blindpipe /app/blindpipe
COPY --from=builder /usr/src/app/models /app/models

ENV BLINDPIPE_PORT=8080
ENV BLINDPIPE_NER_MODEL_PATH=/app/models
ENV RUST_LOG=info
